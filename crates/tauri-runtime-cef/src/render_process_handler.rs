// Render-process hook that natively replaces `window.Notification` at V8 context
// creation time, forwarding notification events to the browser process via IPC.
//
// This runs BEFORE any page script executes (V8 context creation), so the app
// cannot snapshot a clean reference to the original Notification constructor.
// No JavaScript is evaluated — we install native V8 functions directly.

use cef::{rc::*, sys::cef_v8_propertyattribute_t, *};
...
fn send_notification_ipc(
  ...
  args.set_string(
    1,
    Some(&CefString::from(body.as_deref().unwrap_or(""))),
  );
  args.set_string(
    2,
    Some(&CefString::from(icon.as_deref().unwrap_or(""))),
  );
  args.set_string(
    3,
    Some(&CefString::from(tag.as_deref().unwrap_or(""))),
  );

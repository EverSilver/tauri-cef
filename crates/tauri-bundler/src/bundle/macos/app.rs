...
fn create_cef_helpers(
  ...
  // The helper .app bundles embed the same main-app binary. The entry-point macro
  // (`#[tauri::entry_point]`) detects the Chromium subprocess via the `--type=` argv
  // switch and routes execution into `tauri_runtime_cef::run_cef_helper_process`,
  // which installs the native V8 notification hook before any page script runs.
  let main_binary_source = settings.binary_path(settings.main_binary()?);
  if !main_binary_source.exists() {
    return Err(GenericError(format!(
      "main binary not found at {} — CEF helper bundles embed the main binary on macOS",
      main_binary_source.display()
    )));
  }
  ...
  for helper_name in helpers {
    ...
    fs_utils::copy_file(&main_binary_source, &helper_exec)?;
